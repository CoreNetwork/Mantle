...
public class GUICategoryPicker extends InventoryGUI
{
	...
	public void onClick(HumanEntity player, ClickType clickType, int slot)
	{
		...
		if(selectedCategory != null)
		{
			...
			{
				Messages.send(RChestSettings.MESSAGE_COMPASS_CATEGORY_NOT_YET_UNLOCKED.string(), player);
				bukkitPlayer.updateInventory();
			}
			else if(!selectedCategory.isUnderTheLimit(bukkitPlayer) && selectedCategory.isRare())
			{
                Messages.send(RChestSettings.MESSAGE_COMPASS_CATEGORY_LOOTED_ALL.string(), player);
				bukkitPlayer.updateInventory();
			}
			else
			{
